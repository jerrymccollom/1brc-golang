...
func main() {
	...
	for _, city := range citiesByName {
		...
		_, _ = fmt.Fprintf(out, "%s;%d.%.1d;%d.%.1d;%.1f\n", city.Name, city.Min/int64(10), minSign*city.Min%int64(10), city.Max/int64(10), maxSign*city.Max%int64(10), float64(city.Sum)/float64(city.Count)/float64(10))
	}
	_ = out.Flush()
}
...
func processPart(reader *mmap.ReaderAt, readerLen int, first int, last int) map[string]*City {
	cities := make(map[string]*City)
	state := Name
	name := ""
	sign := int64(1)
